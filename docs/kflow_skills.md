...
已启用时，按文件路径定位目标 Node。未受管文件不是错误，也不应因此自动登记。

新文件只有在值得长期记住“从哪里来、变化后影响哪里”时才适合登记，例如长期需求、约束、架构、设计、关键决策或验证结论。临时草稿、缓存、构建产物、可重建文件和大量普通源码通常不登记。无法判断时，保持为普通文件或向人类建议，不静默扩图。
...
`review_order` 是 impact 结果中的建议检查顺序，不是独立操作，也不会自动执行。把 affected 理解为“可能需要检查”，不要理解为“一定错误、必须修改”。
