"""Deterministic, Git-native persistence for KFlow v2 facts."""
...
class StorageError(ValueError):
    """Raised when persisted v2 metadata is missing or malformed."""


def initialize_project(root: Path) -> None:
    """Create an empty schema-v2 metadata tree without touching user files."""
    root = Path(root)
    ...
    for name in ("nodes", "derivations", "confirmations", "cache", "runtime"):
        (metadata / name).mkdir()
    _write_json(
        metadata / "project.json",
        {"kind": "kflow-project", "schema_version": SCHEMA_VERSION},
    )
    (metadata / ".gitignore").write_text(
